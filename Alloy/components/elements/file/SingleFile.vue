...
import fileUpload from '~/components/elements/lib/fileUpload.vue'
...
export default {
	...
	methods: {
		...
		findData () {
			...
			for (const item of this.childrenWithData) {
				...
				if (data.dataFromDatabase) {
					// checking which data has been found --> text or files
					...
					} else if (data.dataFromDatabase.files) {
						// adding data to child
						item.dataOriginal = data.dataFromDatabase.files[0].fileName
						item.dataToEdit = data.dataFromDatabase.files[0].fileName
					}
				} else {
		...
		enterData (event, item) {
			...
			if (item.componentId === 'fileUpload') {
				payload = {
					elementId: item.elementId,
					data: {
						files: [
