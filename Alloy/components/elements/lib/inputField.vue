		...
		dataToEdit(value) {
			...
			this.$apollo.query({
				...
				`,
					...
					this.result = [];
					// checking if the element has to watch for another element
	...
	methods: {
		...
		optionStepper(value) {
			...
			document.getElementsByClassName("options").forEach((element) => {
				...
				document.getElementById("active-option-" + this.elementId + "-" + this.optionStepperCounter).style.color = "#6bbcff";
				document.getElementById("active-option-" + this.elementId + "-" + this.optionStepperCounter).style.backgroundColor = "#1c3349";
				} else {
